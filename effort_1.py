...
true_outcome_function = lambda x: x[:, 0] + x[:, 1] + 2 * x[:, 2] + x[:, 3] - 1
node_function_dict = {
    ...
    'X_1': (lambda x: x, gcm.ScipyDistribution(stats.norm, loc=0, scale=1)),
...
causal_graph = nx.DiGraph([('A', 'Z_A'), ('Z_A_prime', 'X_1'), ('Z_A_prime', 'X_2'), ('Z_A_prime', 'Y'), ('X_1', 'Y'), ('X_2', 'Y'), ('Z_A', 'Y')])
...
opportunity_set = ['X_1', 'X_2', 'Z_A_prime']
opportunity_set_star = [f'{variable_name}_star' for variable_name in opportunity_set]

def individual_equal_effort_lhs_condition(observed_row, protected_attribute, outcome_name):
    query_string = ' & '.join(f'{variable} == {observed_row[variable]}' for variable in observed_row.index)
    return query_string

def individual_equal_effort_rhs_condition(observed_row, protected_attribute, outcome_name):
...
distances_df = compute_loss_between_lhs_vs_rhs(
    ...
    lhs_condition_function=individual_equal_effort_lhs_condition,
    rhs_condition_function=individual_equal_effort_rhs_condition,
...
filename = f"results/effort_s1_{','.join(opportunity_set)},n={dataset_size},nS={num_backtracking_samples},nL={num_loss_function_samples}"
distances_df.to_csv(f'{filename}.csv')
print(f'Saved to: {filename}.csv')
plt.show()

factual_variables = opportunity_set
counterfactual_variables = opportunity_set_star
num_loss_function_samples = 1000
num_runs = 10

all_results_list = []
for model_description, black_box_model, black_box_feature_names in models_to_test:
    for run_number in tqdm(list(range(num_runs)), desc=f'Runs for {model_description}'):
        black_box_model.fit(X=X_train[black_box_feature_names], y=y_train)
        ...
        for outcome_value in [-1, 1]:
            ...
            term_1_query = f'{outcome_name} {observed_outcome_query} & {protected_attribute} == {a_observed}'
            term_2_query = f'{outcome_name}_star {flipped_outcome_query} & {outcome_name} {observed_outcome_query} & {protected_attribute} == {a_observed}'
            term_3_query = f'{outcome_name} {observed_outcome_query} & {protected_attribute} == {a_prime}'
            term_4_query = f'{outcome_name}_star {flipped_outcome_query} & {outcome_name} {observed_outcome_query} & {protected_attribute} == {a_prime}'

            samples_dict = {}
            queries = [term_1_query, term_2_query, term_3_query, term_4_query]
            for query in queries:
                samples = cf_data_with_predictions.query(expr=query)

                if len(samples) < 2:
                    raise RuntimeError(f'Not enough samples for query:\n\t{query}')

                if len(samples) > num_loss_function_samples:
                    samples = samples.sample(num_loss_function_samples)

                samples_dict[query] = samples

            loss_function = SamplesLoss(loss='energy')
            term_1_dist = torch.tensor(
                np.concatenate(
                    [np.expand_dims(samples_dict[term_1_query][name].values, axis=1)
                    for name in factual_variables],
                    axis=1
                )
            )
            term_2_dist = torch.tensor(
                np.concatenate(
                    [np.expand_dims(samples_dict[term_2_query][name].values, axis=1)
                    for name in counterfactual_variables],
                    axis=1
                )
            )
            term_3_dist = torch.tensor(
                np.concatenate(
                    [np.expand_dims(samples_dict[term_3_query][name].values, axis=1)
                    for name in factual_variables],
                    axis=1
                )
            )
            term_4_dist = torch.tensor(
                np.concatenate(
                    [np.expand_dims(samples_dict[term_4_query][name].values, axis=1)
                    for name in counterfactual_variables],
                    axis=1
                )
            )
            distance_between_term_1_term_2 = loss_function(term_1_dist, term_2_dist).item()
            distance_between_term_3_term_4 = loss_function(term_3_dist, term_4_dist).item()

            observed_outcome_string = '$\hat{Y} \leq 0$' if observed_outcome_query == '<= 0' else '$\hat{Y} > 0$'
            description_string_1 = f'$A={a_observed},' + '\hat{Y} \leq 0$' if observed_outcome_query == '<= 0' else f'$A={a_observed},' + '\hat{Y} > 0$'
            description_string_2 = f'$A={a_prime},' + '\hat{Y} \leq 0$' if observed_outcome_query == '<= 0' else f'$A={a_prime},' + '\hat{Y} > 0$'

            result_dicts = [
                {
                    f'{protected_attribute}': a_observed,
                    'group': description_string_1,
                    f'Observed {outcome_name}': observed_outcome_string,
                    'distance': distance_between_term_1_term_2,
                    'model': model_description,
                    'run_number': run_number
                },
                {
                    f'{protected_attribute}': a_prime,
                    'group': description_string_2,
                    f'Observed {outcome_name}': observed_outcome_string,
                    'distance': distance_between_term_3_term_4,
                    'model': model_description,
                    'run_number': run_number
                }
            ]
            all_results_list.extend(result_dicts)
...
sns.set(rc={'text.usetex' : True}, font_scale=2, context='paper')
sns.set_style('whitegrid')
sns.boxplot(y='model', x='distance', hue='group', data=results_df)
sns.despine(right = True)
plt.axvline(x=0, linestyle='dashed', color='r')
plt.ylabel('')
plt.xlabel('Absolute Energy Distance MMD')
...
filename = f"results/effort_s1_group_{','.join(opportunity_set)},n={dataset_size},nS={num_backtracking_samples},nL={num_loss_function_samples}"
